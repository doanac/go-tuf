...
func (rs *RepoSuite) TestStatus(c *C) {
	...
	c.Assert(r.RoleStatus("timestamp", expires), ErrorMatches, "role expired on.*")
	c.Assert(r.RoleStatus("snapshot", expires), IsNil)
	c.Assert(r.RoleStatus("targets", expires), IsNil)
	c.Assert(r.RoleStatus("root", expires), IsNil)
}
